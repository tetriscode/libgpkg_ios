...
static void ST_SRID(sqlite3_context *context, int nbArgs,
  ...
  if (nbArgs == 1) {
    ...
    if (spatialdb->write_blob_header(&FUNCTION_GEOM_ARG_STREAM(geomblob),
                                     &geomblob, FUNCTION_ERROR) != SQLITE_OK) {
      if (error_count(FUNCTION_ERROR) == 0) {
    ...
    sqlite3_result_blob(
        context, binstream_data(&FUNCTION_GEOM_ARG_STREAM(geomblob)),
        (int)binstream_available(&FUNCTION_GEOM_ARG_STREAM(geomblob)),
        SQLITE_TRANSIENT);
  }
...
static void ST_IsEmpty(sqlite3_context *context, int nbArgs,
                       sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_GEOM_ARG(geomblob);
...
static void ST_IsMeasured(sqlite3_context *context, int nbArgs,
                          sqlite3_value **args) {
  spatialdb_t *spatialdb;
  ...
  sqlite3_result_int(context,
                     wkb.coord_type == GEOM_XYM || wkb.coord_type == GEOM_XYZM);

  FUNCTION_END(context);
  FUNCTION_FREE_WKB_ARG(wkb);
}

static void ST_Is3d(sqlite3_context *context, int nbArgs,
                    sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_WKB_ARG(wkb);
  ...
  sqlite3_result_int(context,
                     wkb.coord_type == GEOM_XYZ || wkb.coord_type == GEOM_XYZM);

  FUNCTION_END(context);
  FUNCTION_FREE_WKB_ARG(wkb);
}

static void ST_CoordDim(sqlite3_context *context, int nbArgs,
                        sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_WKB_ARG(wkb);
...
static void ST_GeometryType(sqlite3_context *context, int nbArgs,
                            sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_WKB_ARG(wkb);
...
static void ST_AsBinary(sqlite3_context *context, int nbArgs,
                        sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_GEOM_ARG(geomblob);
  ...
  FUNCTION_RESULT = spatialdb->read_geometry(
      &FUNCTION_GEOM_ARG_STREAM(geomblob), wkb_writer_geom_consumer(&writer),
      FUNCTION_ERROR);

  if (FUNCTION_RESULT == SQLITE_OK) {
    sqlite3_result_blob(context, wkb_writer_getwkb(&writer),
                        (int)wkb_writer_length(&writer), sqlite3_free);
...
static void ST_AsText(sqlite3_context *context, int nbArgs,
                      sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_GEOM_ARG(geomblob);
  ...
  FUNCTION_RESULT = spatialdb->read_geometry(
      &FUNCTION_GEOM_ARG_STREAM(geomblob), wkt_writer_geom_consumer(&writer),
      FUNCTION_ERROR);

  if (FUNCTION_RESULT == SQLITE_OK) {
    sqlite3_result_text(context, wkt_writer_getwkt(&writer),
                        (int)wkt_writer_length(&writer), SQLITE_TRANSIENT);
  }
...
static int geometry_is_assignable(geom_type_t expected, geom_type_t actual,
                                  errorstream_t *error) {
  if (!geom_is_assignable(expected, actual)) {
    const char *expectedName = NULL;
    const char *actualName = NULL;
    if (geom_type_name(expected, &expectedName) == SQLITE_OK &&
        geom_type_name(actual, &actualName) == SQLITE_OK) {
      error_append(error, "Incorrect geometry type. Expected '%d' actual '%s'",
...
typedef int (*geometry_constructor_func)(sqlite3_context *context,
                                         void *user_data,
                                         geom_consumer_t *consumer, int nbArgs,
                                         sqlite3_value **args,
                                         errorstream_t *error);

static void geometry_constructor(sqlite3_context *context,
                                 const spatialdb_t *spatialdb,
                                 geometry_constructor_func constructor,
                                 void *user_data, geom_type_t requiredType,
                                 int nbArgs, sqlite3_value **args) {
  FUNCTION_START_STATIC(context, 256);

  geom_blob_auxdata *geom =
      (geom_blob_auxdata *)sqlite3_get_auxdata(context, 0);
  ...
  if (geom == NULL) {
    ...
    FUNCTION_RESULT =
        constructor(context, user_data, geom_blob_writer_geom_consumer(&writer),
                    nbArgs, args, FUNCTION_ERROR);

    if (FUNCTION_RESULT == SQLITE_OK) {
      if (geometry_is_assignable(requiredType, writer.geom_type,
                                 FUNCTION_ERROR) == SQLITE_OK) {
...
static int geom_from_wkb(sqlite3_context *context, void *user_data,
                         geom_consumer_t *consumer, int nbArgs,
...
static void ST_GeomFromWKB(sqlite3_context *context, int nbArgs,
                           sqlite3_value **args) {
  spatialdb_t *spatialdb = (spatialdb_t *)sqlite3_user_data(context);
  geometry_constructor(context, spatialdb, geom_from_wkb, NULL, GEOM_GEOMETRY,
                       nbArgs, args);
}
...
static int geom_from_wkt(sqlite3_context *context, void *user_data,
                         geom_consumer_t *consumer, int nbArgs,
                         sqlite3_value **args, errorstream_t *error) {
  ...
  FUNCTION_RESULT =
      wkt_read_geometry(wkt, FUNCTION_TEXT_ARG_LENGTH(wkt), consumer,
                        (i18n_locale_t *)user_data, FUNCTION_ERROR);
...
static void ST_GeomFromText(sqlite3_context *context, int nbArgs,
                            sqlite3_value **args) {
  fromtext_t *fromtext = (fromtext_t *)sqlite3_user_data(context);
  geometry_constructor(context, fromtext->spatialdb, geom_from_wkt,
                       fromtext->locale, GEOM_GEOMETRY, nbArgs, args);
}

static int point_from_coords(sqlite3_context *context, void *user_data,
                             geom_consumer_t *consumer, int nbArgs,
                             sqlite3_value **args, errorstream_t *error) {
...
static void ST_Point(sqlite3_context *context, int nbArgs,
                     sqlite3_value **args) {
  fromtext_t *fromtext = (fromtext_t *)sqlite3_user_data(context);
  if (sqlite3_value_type(args[0]) == SQLITE_TEXT) {
    geometry_constructor(context, fromtext->spatialdb, geom_from_wkt,
                         fromtext->locale, GEOM_POINT, nbArgs, args);
  } else if (sqlite3_value_type(args[0]) == SQLITE_BLOB) {
    geometry_constructor(context, fromtext->spatialdb, geom_from_wkb, NULL,
                         GEOM_POINT, nbArgs, args);
  } else {
    geometry_constructor(context, fromtext->spatialdb, point_from_coords, NULL,
                         GEOM_POINT, nbArgs, args);
  }
}

static void GPKG_IsAssignable(sqlite3_context *context, int nbArgs,
                              sqlite3_value **args) {
  ...
  if (FUNCTION_RESULT != SQLITE_OK) {
    error_append(FUNCTION_ERROR, "Invalid geometry type %s",
                 expected_type_name);
    goto exit;
  }
...
static void GPKG_SpatialDBType(sqlite3_context *context, int nbArgs,
                               sqlite3_value **args) {
  spatialdb_t *spatialdb;
...
static void GPKG_CheckSpatialMetaData(sqlite3_context *context, int nbArgs,
                                      sqlite3_value **args) {
  spatialdb_t *spatialdb;
  ...
  FUNCTION_RESULT =
      spatialdb->check_meta(FUNCTION_DB_HANDLE, db_name, check, FUNCTION_ERROR);
  if (FUNCTION_RESULT == SQLITE_OK) {
    sqlite3_result_null(context);
  }
...
static void GPKG_InitSpatialMetaData(sqlite3_context *context, int nbArgs,
                                     sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_TEXT_ARG(db_name);
  ...
  FUNCTION_RESULT =
      spatialdb->init_meta(FUNCTION_DB_HANDLE, db_name, FUNCTION_ERROR);
 ...
 */
static void GPKG_AddGeometryColumn(sqlite3_context *context, int nbArgs,
                                   sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_TEXT_ARG(db_name);
  ...
  if (FUNCTION_RESULT == SQLITE_OK) {
    FUNCTION_RESULT = spatialdb->add_geometry_column(
        FUNCTION_DB_HANDLE, db_name, table_name, column_name, geometry_type,
        srs_id, z, m, FUNCTION_ERROR);
  }
...
static void GPKG_CreateTilesTable(sqlite3_context *context, int nbArgs,
                                  sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_TEXT_ARG(db_name);
  FUNCTION_TEXT_ARG(table_name);
  ...
  if (spatialdb->create_tiles_table == NULL) {
    error_append(FUNCTION_ERROR, "Tiles tables are not supported in %s mode",
                 spatialdb->name);
    goto exit;
  }
  ...
  FUNCTION_RESULT =
      spatialdb->init_meta(FUNCTION_DB_HANDLE, db_name, FUNCTION_ERROR);
  if (FUNCTION_RESULT == SQLITE_OK) {
    FUNCTION_RESULT = spatialdb->create_tiles_table(FUNCTION_DB_HANDLE, db_name,
                                                    table_name, FUNCTION_ERROR);
  }
...
static void GPKG_CreateSpatialIndex(sqlite3_context *context, int nbArgs,
                                    sqlite3_value **args) {
  spatialdb_t *spatialdb;
  FUNCTION_TEXT_ARG(db_name);
  FUNCTION_TEXT_ARG(table_name);
  ...
  if (spatialdb->create_spatial_index == NULL) {
    error_append(FUNCTION_ERROR, "Spatial indexes are not supported in %s mode",
                 spatialdb->name);
    goto exit;
  }
  ...
  FUNCTION_RESULT =
      spatialdb->init_meta(FUNCTION_DB_HANDLE, db_name, FUNCTION_ERROR);
  if (FUNCTION_RESULT == SQLITE_OK) {
    FUNCTION_RESULT = spatialdb->create_spatial_index(
        FUNCTION_DB_HANDLE, db_name, table_name, geometry_column_name,
...
const spatialdb_t *spatialdb_detect_schema(sqlite3 *db) {
  ...
  const spatialdb_t *schemas[] = {
      spatialdb_geopackage_schema(), spatialdb_spatialite4_schema(),
      spatialdb_spatialite3_schema(), spatialdb_spatialite2_schema(), NULL};

  const spatialdb_t **schema = &schemas[0];
  while (*schema != NULL) {
    error_reset(&error);
    (*schema)->check_meta(db, "main",
                          SQL_CHECK_PRIMARY_KEY | SQL_CHECK_NULLABLE, &error);
...
#define SPATIALDB_FUNCTION(db, pre, name, args, flags, spatialdb, err)         \
  do {                                                                         \
    sql_create_function(db, STR(name), pre##_##name, args, flags,              \
                        (void *)spatialdb, NULL, err);                         \
    sql_create_function(db, STR(pre##_##name), pre##_##name, args, flags,      \
                        (void *)spatialdb, NULL, err);                         \
  } while (0)

#define SPATIALDB_ALIAS(db, pre, name, func, args, flags, spatialdb, err)      \
  do {                                                                         \
    sql_create_function(db, STR(name), pre##_##func, args, flags,              \
                        (void *)spatialdb, NULL, err);                         \
    sql_create_function(db, STR(pre##_##name), pre##_##func, args, flags,      \
                        (void *)spatialdb, NULL, err);                         \
  } while (0)
...
#define FROMTEXT_FUNCTION(db, pre, name, args, flags, ft, err)                 \
  do {                                                                         \
    ...
    sql_create_function(db, STR(name), pre##_##name, args, flags, ft,          \
                        (void (*)(void *))fromtext_release, err);              \
    fromtext_acquire(fromtext);                                                \
    sql_create_function(db, STR(pre##_##name), pre##_##name, args, flags, ft,  \
                        (void (*)(void *))fromtext_release, err);              \
  } while (0)
...
#define FROMTEXT_ALIAS(db, pre, name, func, args, flags, ft, err)              \
  do {                                                                         \
    fromtext_acquire(fromtext);                                                \
    sql_create_function(db, STR(name), pre##_##func, args, flags, (void *)ft,  \
                        (void (*)(void *))fromtext_release, err);              \
    fromtext_acquire(fromtext);                                                \
    sql_create_function(db, STR(pre##_##name), pre##_##func, args, flags,      \
                        (void *)ft, (void (*)(void *))fromtext_release, err);  \
  } while (0)

SQLITE_EXTENSION_INIT1

int spatialdb_init(sqlite3 *db, const char **pzErrMsg,
  ...
  if (sqlite3_libversion_number() < 3007000) {
    if (pzErrMsg) {
      *pzErrMsg = sqlite3_mprintf(
          "libgpkg requires SQLite 3.7.0 or higher; detected %s",
          sqlite3_libversion());
    }
    return SQLITE_ERROR;
  }
  ...
  if (1) {
    ...
    while (*forbidden_option != NULL) {
      if (sqlite3_compileoption_used(*forbidden_option)) {
        if (pzErrMsg) {
          *pzErrMsg = sqlite3_mprintf(
              "libgpkg requires %s support but %s compile option was used",
    ...
    const char *required_options[] = {"SQLITE_ENABLE_RTREE", "rtree", NULL,
                                      NULL};
    ...
    while (*required_option != NULL) {
      if (!sqlite3_compileoption_used(*required_option)) {
        if (pzErrMsg) {
          *pzErrMsg = sqlite3_mprintf(
              "libgpkg requires %s support but %s compile option was not used",
  ...
  SPATIALDB_FUNCTION(db, ST, IsMeasured, 1, SQL_DETERMINISTIC, spatialdb,
                     &error);
  SPATIALDB_FUNCTION(db, ST, CoordDim, 1, SQL_DETERMINISTIC, spatialdb, &error);
  SPATIALDB_FUNCTION(db, ST, GeometryType, 1, SQL_DETERMINISTIC, spatialdb,
                     &error);
  SPATIALDB_FUNCTION(db, ST, AsBinary, 1, SQL_DETERMINISTIC, spatialdb, &error);
  SPATIALDB_FUNCTION(db, ST, GeomFromWKB, 1, SQL_DETERMINISTIC, spatialdb,
                     &error);
  SPATIALDB_FUNCTION(db, ST, GeomFromWKB, 2, SQL_DETERMINISTIC, spatialdb,
                     &error);
  SPATIALDB_ALIAS(db, ST, WKBToSQL, GeomFromWKB, 1, SQL_DETERMINISTIC,
                  spatialdb, &error);
  SPATIALDB_ALIAS(db, ST, WKBToSQL, GeomFromWKB, 2, SQL_DETERMINISTIC,
                  spatialdb, &error);
  SPATIALDB_FUNCTION(db, ST, AsText, 1, SQL_DETERMINISTIC, spatialdb, &error);

  fromtext_t *fromtext = fromtext_init(spatialdb);
  if (fromtext != NULL) {
    FROMTEXT_FUNCTION(db, ST, GeomFromText, 1, SQL_DETERMINISTIC, fromtext,
                      &error);
    FROMTEXT_FUNCTION(db, ST, GeomFromText, 2, SQL_DETERMINISTIC, fromtext,
                      &error);
    FROMTEXT_ALIAS(db, ST, WKTToSQL, GeomFromText, 1, SQL_DETERMINISTIC,
                   fromtext, &error);
    FROMTEXT_ALIAS(db, ST, WKTToSQL, GeomFromText, 2, SQL_DETERMINISTIC,
                   fromtext, &error);

    FROMTEXT_FUNCTION(db, ST, Point, 1, SQL_DETERMINISTIC, fromtext, &error);
    FROMTEXT_ALIAS(db, ST, MakePoint, Point, 1, SQL_DETERMINISTIC, fromtext,
                   &error);
    FROMTEXT_FUNCTION(db, ST, Point, 2, SQL_DETERMINISTIC, fromtext, &error);
    FROMTEXT_ALIAS(db, ST, MakePoint, Point, 2, SQL_DETERMINISTIC, fromtext,
                   &error);
    FROMTEXT_FUNCTION(db, ST, Point, 3, SQL_DETERMINISTIC, fromtext, &error);
    FROMTEXT_ALIAS(db, ST, MakePoint, Point, 3, SQL_DETERMINISTIC, fromtext,
                   &error);
    FROMTEXT_FUNCTION(db, ST, Point, 4, SQL_DETERMINISTIC, fromtext, &error);
    FROMTEXT_ALIAS(db, ST, MakePoint, Point, 4, SQL_DETERMINISTIC, fromtext,
                   &error);
    FROMTEXT_FUNCTION(db, ST, Point, 5, SQL_DETERMINISTIC, fromtext, &error);
    FROMTEXT_ALIAS(db, ST, MakePoint, Point, 5, SQL_DETERMINISTIC, fromtext,
                   &error);
  ...
  SPATIALDB_FUNCTION(db, GPKG, IsAssignable, 2, SQL_DETERMINISTIC, spatialdb,
                     &error);
