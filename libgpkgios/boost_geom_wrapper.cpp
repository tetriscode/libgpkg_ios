...
struct geometry_ptr_visitor
    : boost::static_visitor<typename Visitor::result_type> {
...
int is_valid(GeometryPtr &geometry) {
  return boost::apply_visitor(
      detail::geometry_ptr_visitor<detail::is_valid>(true), geometry);
}

int is_simple(GeometryPtr &geometry) {
  return boost::apply_visitor(
      detail::geometry_ptr_visitor<detail::is_simple>(true), geometry);
}

double area(GeometryPtr &geometry) {
  return boost::apply_visitor(detail::geometry_ptr_visitor<detail::area>(0.0),
                              geometry);
}
}